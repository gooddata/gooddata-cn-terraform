...
`enable_ai_features = true` deploys the GenAI services, but GoodData.CN still needs an LLM provider configured per organization before the AI assistant works. On AWS, set `enable_bedrock_llm = true` in your `settings.tfvars` to wire this up end to end:
  ...
  - Terraform reuses an existing Bedrock IAM user when it finds one (given explicitly via `bedrock_iam_user_name`, or auto-discovered by the `Purpose=gdcn-genai-bedrock` tag, which marks users provisioned outside Terraform for sharing) and only mints a fresh access key for it; otherwise it creates a dedicated IAM user (`modules/bedrock`) whose only permissions are invoking and enumerating Bedrock models. Auto-discovery runs only until the deployment has a user of its own, and users created here carry a distinct `Purpose=gdcn-genai-bedrock-managed` tag, so deployments never adopt each other's Terraform-managed users. Reused users are never destroyed by this stack, must already carry Bedrock invoke permissions, and need a free access key slot (IAM allows two per user).
