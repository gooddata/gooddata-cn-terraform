...
locals {
  ...
  # so no username/password is configured for such a host.
  ...
  gdcn_chart_repo_is_private = local.use_gdcn_registry_auth && strcontains(var.helm_gdcn_repository, var.gdcn_registry_server)
}
