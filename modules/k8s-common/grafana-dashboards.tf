...
resource "kubernetes_config_map_v1" "grafana_dashboard_quiver_shards_overview" {
  count = var.enable_observability ? 1 : 0

  metadata {
    name      = "grafana-dashboard-quiver-shards-overview"
    namespace = kubernetes_namespace_v1.observability[0].metadata[0].name
    labels = {
      grafana_dashboard = "1"
    }
  ...
  data = {
    "quiver-shards-overview.json" = file("${path.module}/dashboards/quiver-shards-overview.json")
  }
