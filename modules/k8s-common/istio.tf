...
#
# When enable_istio = true (AWS ALB mode), we install Istio and expose the
# ingress gateway as NodePort so the ALB can forward to it (target-type=instance).
###

locals {
  istio_enabled       = var.enable_istio && var.cloud == "aws" && var.ingress_controller == "alb"
...
resource "tls_self_signed_cert" "alb_to_istio" {
  ...
  dns_names = compact([
    ...
    trimspace(var.base_domain) != "" ? "*.${trimspace(var.base_domain)}" : null
  ])
