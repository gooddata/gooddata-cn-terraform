...
lb_arn=$(awsx elbv2 describe-load-balancers \
  ...
  --output text 2>&1) || {
  # The controller can win the race between the check above and this lookup.
  if grep -q "LoadBalancerNotFound" <<<"${lb_arn}"; then
    echo "ALB '${LB_NAME}' disappeared during the ARN lookup."
    wait_for_enis
    exit 0
  fi
