...
if np_err=$(kc get crd nodepools.karpenter.sh 2>&1 >/dev/null); then
  echo "Deleting Karpenter NodePools so their NodeClaims cascade..."
  # Not masked: a surviving NodePool lets Karpenter replace the instances we are
  # about to terminate.
  if ! del_err=$(kc delete nodepools --all --wait=false 2>&1 >/dev/null); then
    echo "ERROR: could not delete NodePools on '${CLUSTER_NAME}': ${del_err}" >&2
    exit 1
  fi
elif ! grep -qi "not found" <<<"${np_err}"; then
  echo "ERROR: cannot read the NodePool CRD on '${CLUSTER_NAME}': ${np_err}" >&2
  exit 1
fi
...
count_claims() {
  local out
  # -o name prints one resource per line and nothing at all when the collection
  # is empty, so the count never depends on kubectl's human-readable notices.
  out=$(kc get nodeclaims -o name 2>/dev/null) || return 1
  if [[ -z "${out}" ]]; then echo 0; else grep -c . <<<"${out}"; fi
}
...
if ! claims=$(kc get nodeclaims -o name 2>/dev/null); then
  echo "ERROR: cannot list NodeClaims to clear their finalizers on '${CLUSTER_NAME}'." >&2
  exit 1
fi

failed=0
for nc in ${claims}; do
  if ! patch_err=$(kc patch "${nc}" --type=merge -p '{"metadata":{"finalizers":null}}' 2>&1 >/dev/null); then
    echo "WARNING: could not clear the finalizer on ${nc}: ${patch_err}" >&2
    failed=1
  fi
done

if ((failed)); then
  echo "ERROR: some NodeClaim finalizers remain; VPC deletion will block until they are cleared." >&2
  exit 1
fi
